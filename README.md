...
A small floating box for switching specializations at a glance. One button per
...
- `/talented lock` / `/talented unlock` - lock or unlock the frame for moving
- `/talented reset` - reset the frame to its default position
- `/talented size small|medium|large` - change icon size
- `/talented debug` - print diagnostic info (spec count, current spec, etc.)
